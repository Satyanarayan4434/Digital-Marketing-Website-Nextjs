...
export default function SliderDesktop() {
  ...
  const getCardStyle = (index) => {
    ...
    return {
      ...
      `,
      filter: `blur(${absOffset === 0 ? 0 : absOffset === 1 ? '3px' : '6px'})`,
    ...
    <div className="relative h-[600px] w-full max-w-7xl mx-auto flex items-center justify-center overflow-hidden">
          ...
          >
            <div className="relative h-full w-full">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-4 z-10">
                ...
                <p className="text-gray-100 text-lg text-center">{card.content}</p>
              </div>
            </div>
          </motion.div>
        );
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-3">
        {cards.map((card, index) => (
          <button
            key={card.id}
            className={`w-3 h-3 rounded-full transition-all duration-300 ${index === current ? 'bg-[#040406] scale-150' : 'bg-white'}`}
